...
Write-Output "";
$csvFile = read-host " Name of the CSV file (i.e.: C:\users.csv)"
...
$users = Import-Csv $csvFile
            ...
            Write-Output "";
            Write-Host " Enter a correct CSV file" -ForegroundColor DarkRed
        }
        ...
        if ($user.manager -eq "") # In case it's a service account or a boss
            {
                try {
                    ...
                    {
                        Write-Output "Could not create the user $($user.SamAccountName), $_"
                    }
            }
            else
             {
