...
$datos = Get-Content ".\scripts\obj\gui\docs\deshabilitarusu.txt"


        try {
            Get-ADUser -Filter "Name -eq '$datos'" > .\scripts\obj\gui\docs\deshabilitarusu.txt
            
            $contador = (Get-Content .\scripts\obj\gui\docs\deshabilitarusu.txt | where{$_-match"$datos"}).Count

            if ($contador -ne 0) {

                Disable-ADAccount -Identity $datos
