...
$datos = Get-Content ".\scripts\obj\gui\docs\exportarusucsv.txt"

$csv = ".csv"

$contador = (Get-Content ".\scripts\obj\gui\docs\exportarusucsv.txt" | where{$_-match"$csv"}).Count
...
try {
Get-ADUser -Filter * -Properties * | Export-Csv $datos
#Write-Host " $ruta generado correctamente" -ForegroundColor DarkGreen
echo 0 > .\scripts\obj\gui\docs\compcsvusu.txt
}
            ...
            echo 100 > .\scripts\obj\gui\docs\compcsvusu.txt
        }
